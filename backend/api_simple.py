"""
Simple API version without advanced features
เพื่อให้ระบบทำงานได้ก่อน
"""
import sys
import os
import logging
...
async def predict(front: UploadFile = File(...), back: Optional[UploadFile] = File(None)):
    try:
        ...
        # เรียกใช้ valuation และ recommendation
...
def root():
    ...
    return {"message": "Amulet-AI API is running", "status": "ok", "ai_mode": "advanced_simulation"}

@app.get("/health")
async def health_check():
    """Health check endpoint"""
    return {"status": "healthy", "message": "Amulet-AI API is running", "ai_mode": "advanced_simulation"}
...
async def get_system_status():
    ...
    elif hasattr(model_loader, 'use_advanced_simulation') and model_loader.use_advanced_simulation:
...
# รัน: uvicorn backend.api_simple:app --reload --port 8000
